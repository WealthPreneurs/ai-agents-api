...
async function checkAiVisibility(apiKey, businessName, category, city) {
  ...
  // Run both queries concurrently — this endpoint typically runs inside a
  // serverless function with a tight execution timeout, and two sequential
  // Claude calls plus the Yelp lookup can otherwise run past it.
  return Promise.all(queries.map(async (query) => {
    ...
    return { query, mentioned, note, rawAnswer: answer };
  }));
}
